...
export default function BottomNavigation({ className = "" }: BottomNavigationProps) {
  ...
  return (
    <View
      className={`absolute bottom-0 left-0 right-0 bg-[#121212] border-t border-[#1E1E1E] ${className}`}
    >
      <View className="flex-row justify-around py-3">
        {tabs.map((tab) => (
          <TouchableOpacity
            key={tab.name}
            className={`items-center px-4 py-2 rounded-full ${activeTab === tab.name ? "bg-[#1E1E1E]" : ""}`}
            onPress={() => {
          ...
          >
            ...
            <Text
              className={`text-xs mt-1 ${activeTab === tab.name ? "text-[#1ED760]" : "text-[#B3B3B3]"}`}
            >
              {tab.title}
            </Text>
          </TouchableOpacity>
